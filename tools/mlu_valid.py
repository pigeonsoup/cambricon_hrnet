...
from my_transform import ToTensorNoDiv255, ToHalfTensorNoDiv255
...
parser.add_argument('--data_dir', type=str, help="directory to testset", required=True)
...
def valid(model, means, stds, device):
    ...
    if device == "cpu":
    ...
    elif device == ct.mlu_device():
        if args.use_half:
            valid_loader = torch.utils.data.DataLoader(
                datasets.ImageFolder(args.data_dir, transforms.Compose([
                        transforms.Resize(int(config.MODEL.IMAGE_SIZE[0] / 0.875)),
                        transforms.CenterCrop(config.MODEL.IMAGE_SIZE[0]),
                        ToHalfTensorNoDiv255()])),
                # batch_size=config.TEST.BATCH_SIZE_PER_GPU*len(gpus),
                batch_size=config.TEST.BATCH_SIZE_PER_GPU,
                shuffle=False,
                num_workers=config.WORKERS,
        ...
        raise NameError("Device name not supported.")

    # evaluate on validation set
    validate(None, valid_loader, model, criterion, None, None, None, device)
...
def main():
    ...
    if args.device == "cpu":
        ...
        valid(net_traced, mean, std, ct.mlu_device())
